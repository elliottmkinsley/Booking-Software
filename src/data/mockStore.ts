/**
 * In-browser stand-in for the future database.
 *
 * Everything the demo "saves" lives in this object, then is copied to
 * sessionStorage so a refresh keeps your edits. Closing the tab wipes it and
 * the next visit starts from `mockData` again.
 *
 * When Azure exists, replace the repository modules in `data/repositories/`
 * with HTTP clients — do not import this store from presentation or business.
 * Bump STORAGE_KEY (v18, v19, …) if the shape changes so old
 * saved data is discarded instead of crashing.
 */
import {
  seedAccounts,
  seedActivities,
...
/** True when saved JSON still matches the fields this app version expects. */
...
/** Load from sessionStorage, or start fresh from seed data if missing/stale. */
...
/** Write the in-memory store back to this tab's sessionStorage. */
export function persist(): void {
  sessionStorage.setItem(STORAGE_KEY, JSON.stringify(store));
}

let idCounter = Date.now();
/** Unique ids for new rows (`eq-…`, `lab-…`, `booking-…`, …). */
