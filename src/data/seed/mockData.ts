/**
 * Sample data that fills the demo so screens are not empty.
 *
 * Invented for a remote-sensing lab (drones, LiDAR, spectrometers, GIS).
 * Names, emails, and user-guide links are placeholders — swap this file for
 * a database once Azure is connected. `mockStore` copies these arrays on
 * first load, then keeps later edits in sessionStorage.
 */
...
/** Physical labs shown on the main menu. */
...
/** Staff used as equipment owners (separate from directory Accounts for now). */
...
/** Catalog of trainings someone may need before reserving gear. */
...
/** Instruments (tied to a lab) and shared software (labId is null). */
...
/** Directory accounts who can sign people off on trainings. */
...
/** People directory (searchable when adding managers or trainers). */
...
/** Accounts that have the lab-manager role. */
...
/** Which labs each manager owns (account id → lab ids). */
...
/** Supplies that can run low (lab-wide or tied to one instrument). */
...
/** A few existing reservations so the calendar is not blank. */
...
/** Starter audit-log rows so the Activity modal has something to show. */
